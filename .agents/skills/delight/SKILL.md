...
1. If the codebase answers the question with high confidence, proceed.
2. If critical context is missing or confidence is medium or lower, ask the user directly in chat before changing the design.
3. If you must move with assumptions, state them explicitly and keep the change narrowly scoped.

Do NOT proceed with speculative design changes when the missing context would materially change the outcome. Delight that's wrong for the context is worse than no delight at all.
...
If any of these are unclear from the codebase, stop and ask the user directly in chat before making the visual change.
