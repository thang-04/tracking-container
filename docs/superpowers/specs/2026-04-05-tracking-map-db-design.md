...
- Dùng các API/read-service tách theo nghiệp vụ, rồi compose lại cho `/map` và preview ở dashboard.
...
Dùng các API/read-service server-side tách theo nghiệp vụ để đọc dữ liệu từ Prisma, sau đó có một compose layer cho tracking UI chuẩn hóa chúng thành payload dùng cho:
...
- Không biến logic đọc dữ liệu thành một khối `/map` monolith.
...
- `lib/master-data/get-ports.ts`: đọc cảng dùng cho marker và bộ lọc
- `lib/master-data/get-routes.ts`: đọc tuyến và checkpoint
...
## 6. Thiết kế API theo nghiệp vụ

### 6.1. Nguyên tắc

Không dùng một API duy nhất kiểu `/map` làm nguồn chân lý cho toàn bộ nghiệp vụ tracking.

Thay vào đó:

- mỗi nghiệp vụ có API/read-service riêng
- màn `/map` chỉ là consumer
- `get-tracking-overview.ts` chỉ là lớp compose cho screen, không phải nơi thay thế các API nghiệp vụ

Trong phạm vi hiện tại, `API` được hiểu là application API/read-service bên trong Next.js app. Nếu cần mở ra HTTP route handlers sau này, các route handler đó phải wrap đúng các service này thay vì viết lại query riêng.
...
- `lib/master-data/get-ports.ts`
- `lib/master-data/get-routes.ts`

HTTP path tương ứng nếu cần public sau này:

- `/api/master-data/ports`
- `/api/master-data/routes`
...
- gọi các API/read-service nghiệp vụ
- merge dữ liệu
- tính `x/y`
- gắn route summary, manifest count, container count
- trả DTO tối ưu cho UI

HTTP path tổng hợp như `/api/tracking/overview` chỉ là tùy chọn về sau cho client ngoài route server-side. Nó không thay thế các API/read-service nghiệp vụ bên dưới.

## 7. Mapping dữ liệu từ database sang UI

### 7.1. Location markers
...
## 8. Quy ước nghiệp vụ trong UI

### 8.1. Bỏ field không có trong schema
...
## 10. Empty state và demo data

### 10.1. Runtime behavior
...
## 11. Cách chia implementation bằng 5 agent
...
Thiết kế này giữ nguyên route framework của Next.js nhưng tách logic tracking ra các file tên theo nghiệp vụ, đủ ngắn và rõ nghĩa. Dữ liệu map sẽ được lấy từ DB thông qua các API/read-service tách theo nghiệp vụ, sau đó compose lại cho `/map` và dashboard preview, loại bỏ hoàn toàn hardcode runtime trong component.
